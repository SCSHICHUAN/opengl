...
int main(int argc, const char * argv[]) {
    ...
    while(!glfwWindowShouldClose(window))//glfwWindowShouldClose 关闭事件
    {
        ...
        float x = sin(glfwGetTime()) * 3.5;
        float y = sin(glfwGetTime()) * 3.5;
        float z = cos(glfwGetTime()) * 3.5;
...
//        }
        ...
        lightingShader.use();
        lightingShader.setVec3("light.position", camera.Position); //灯的位置
        lightingShader.setVec3("light.direction", camera.Front);//平行光方向 
        lightingShader.setFloat("light.cutOff",   glm::cos(glm::radians(3.f)));//切光角
        lightingShader.setFloat("light.outerCutOff", glm::cos(glm::radians(4.f)));//外切光角
